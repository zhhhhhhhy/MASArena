#!/bin/bash
# Multi-Agent System Benchmark Runner Script
# Usage: ./scripts/run_mas_benchmark.sh [limit]
...
# Define MAS types
MAS_TYPES=("supervisor_mas" "swarm" "agentverse")
...
# Create log file
LOG_FILE="logs/mas_benchmark_$(date +%Y%m%d_%H%M%S).log"
touch $LOG_FILE

echo "Starting Multi-Agent System Benchmark" | tee -a $LOG_FILE
echo "====================================================" | tee -a $LOG_FILE
...
for TASK_CATEGORY in "${TASK_CATEGORIES[@]}"; do
  ...
  for TASK_NAME in "${TASKS[@]}"; do
    # Loop through MAS types
    for MAS_TYPE in "${MAS_TYPES[@]}"; do
      
      echo "Task Category: $TASK_CATEGORY, Task: $TASK_NAME, MAS Type: $MAS_TYPE" | tee -a $LOG_FILE
      ...
      for MODEL_NAME in "${REASONING_MODELS[@]}"; do
        ...
        RESULTS_DIR="results/${TASK_CATEGORY}_${TASK_NAME}/${MAS_TYPE}/${MODEL_PROVIDER}_${MODEL_NAME}/reasoning"
        mkdir -p "$RESULTS_DIR"
        
        # Run the benchmark with reasoning
        python main.py \
          --benchmark "$TASK_NAME" \
          --agent-system "$MAS_TYPE" \
          --limit "$LIMIT" \
      ...
      for MODEL_NAME in "${NON_REASONING_MODELS[@]}"; do
        ...
        # Define results directory
        RESULTS_DIR="results/${TASK_CATEGORY}_${TASK_NAME}/${MAS_TYPE}/${MODEL_PROVIDER}_${MODEL_NAME}/no_reasoning"
        mkdir -p "$RESULTS_DIR"

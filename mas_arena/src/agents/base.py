...
import glob
import time
from mas_arena.src.agents.format_prompts import get_format_prompt
...
class AgentSystem(abc.ABC):
    ...
    def _initialize_evaluator(self, evaluator_type: Type = None):
        ...
        if evaluator_type is None:
            ...
            try:
                from mas_arena.src.evaluators import AVAILABLE_EVALUATORS
    ...
    def _initialize_metrics_collector(self):
        ...
        try:
            from mas_arena.src.metrics.collector import MetricsCollector
    ...
    def generate_benchmark_visualization(self, run_results, summary_data=None, output_dir=None):
        ...
        """
        try:
            from mas_arena.src.visualization.mas_visualizer import BenchmarkVisualizer
        except ImportError:
...
def create_agent_system(name: str, config: Dict[str, Any] = None) -> Optional[AgentSystem]:
    ...
    if config and (config.get("use_tools") or config.get("use_mcp_tools")):
        try:
            from mas_arena.src.tools.tool_integration import ToolIntegrationWrapper
